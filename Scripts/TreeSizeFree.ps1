...
function Install-Program {
    ...
    try {
        ...
        $installerLink = ($htmlContent.Links | Where-Object { $_.href -match "TreeSizeFreeSetup.*\.exe$" }).href | Select-Object -First 1

        if (-not $installerLink) {
            Write-Log "Error: Unable to find the TreeSize Free installer link."
        ...
        $installerPath = Join-Path $TempDir "treesizefree_latest_installer.exe"
