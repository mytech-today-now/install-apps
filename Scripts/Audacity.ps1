...
function Install-Program {
    ...
    try {
        ...
        # audacity-win-...-x64.exe
        $installerLink = ($htmlContent.Links | Where-Object { $_.href -match "audacity-win-.*-x64\.exe$" }).href | Select-Object -First 1

        if (-not $installerLink) {
            Write-Log "Error: Unable to find the Audacity installer link."
        ...
        $installerPath = Join-Path $TempDir "audacity_latest_installer.exe"
